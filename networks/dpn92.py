...
def adaptive_avgmax_pool2d(x, pool_type='avg', padding=0,
    ...
    if pool_type == 'avgmaxc':
        ...
        if pool_type != 'avg':
            print('Invalid pool type %s specified. '
                  'Defaulting to average pooling.' % pool_type)
        x = F.avg_pool2d(
            x, kernel_size=(x.size(2), x.size(3)),
...
class AdaptiveAvgMaxPool2d(torch.nn.Module):
    ...
    def forward(self, x):
        if self.pool_type == 'avgmaxc':
            ...
            x = 0.5 * torch.sum(
                torch.stack([p(x) for p in self.pool]), 0).squeeze(dim=0)
        else:
            x = self.pool(x)
        return x
...
class DPN(nn.Module):
    ...
    def forward(self, x):
        ...
        #return out, features
        return features[3], features[2], features[1], features[0]
